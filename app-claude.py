"""
app.py — AI Pioneers Week 3 Demo
IT Operations Chatbot with Conversation Memory
...
Deploy to Hugging Face Spaces:
    ...
    Add ANTHROPIC_API_KEY to Spaces secrets
"""
...
if "model" not in st.session_state:
    st.session_state.model = "claude-opus-4-5"
...
client = get_client(os.environ.get("ANTHROPIC_API_KEY", ""))
...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "Model",
        options=["claude-opus-4-5", "claude-sonnet-4-5"],
        index=0,
        label_visibility="collapsed",
    )
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px">
    ...
    <div>
        ...
        &nbsp;
        <span class="badge badge-gold">{model_choice.split("-")[1].capitalize()}</span>
        &nbsp;
...
if not client:
    ...
    <div style="background:#0D1F35; border:1px solid #F59E0B; border-radius:12px; padding:20px; margin:20px 0">
        ...
        <div style="color:#94A3B8; font-size:13px; line-height:1.6">
            Set your <code>ANTHROPIC_API_KEY</code> environment variable to use this chatbot.<br><br>
            <strong style="color:white">Local:</strong> Add to a <code>.env</code> file or run:<br>
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            st.caption(f"↑ {final_msg.usage.input_tokens} tokens sent · {final_msg.usage.output_tokens} tokens received")

        except anthropic.AuthenticationError:
            full_response = "❌ Invalid API key. Check your `ANTHROPIC_API_KEY` environment variable."
