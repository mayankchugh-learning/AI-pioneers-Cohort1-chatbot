"""
app-gemini.py — AI Pioneers Week 3 Demo
IT Operations Chatbot with Conversation Memory (Google Gemini)
Mayank Chugh | Azure OpenAI & GenAI Architecture | 2026

Run locally:
    streamlit run app-gemini.py

Deploy to Hugging Face Spaces:
    Upload this file + requirements.txt
    Add GEMINI_API_KEY to Spaces secrets
"""
...
client = get_client(os.environ.get("GEMINI_API_KEY", ""))


def to_gemini_contents(messages: list[dict]) -> list[types.Content]:
...
if not client:
    ...
    <div style="background:#0D1F35; border:1px solid #F59E0B; border-radius:12px; padding:20px; margin:20px 0">
        ...
        <div style="color:#94A3B8; font-size:13px; line-height:1.6">
            Set your <code>GEMINI_API_KEY</code> environment variable to use this chatbot.<br><br>
...
if user_input:
    ...
    # Build Gemini contents — full history every call
    # IT analogy: stateless API, stateful YOU — send full transcript every time
    gemini_contents = to_gemini_contents(st.session_state.messages)
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            if e.code == 401:
                full_response = "❌ Invalid API key. Check your `GEMINI_API_KEY` environment variable."
