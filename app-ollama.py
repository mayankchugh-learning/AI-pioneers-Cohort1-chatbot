"""
app-ollama.py — AI Pioneers Week 3 Demo
IT Operations Chatbot with Conversation Memory (Ollama)
...
Optional env vars:
    ...
    OLLAMA_API_KEY   (default: ollama — not required for local use)
"""
...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "Model",
        options=["llama3.2", "llama3.1:8b", "mistral", "phi3", "gemma2"],
        index=0,
        label_visibility="collapsed",
    )
...
# Ollama setup hint
st.markdown(f"""
<div style="background:#0D1F35; border:1px solid #1E3A5A; border-radius:12px; padding:16px; margin-bottom:20px">
    <div style="font-size:13px; color:#94A3B8; line-height:1.6">
        <strong style="color:white">Ollama (local):</strong> Run <code>ollama serve</code>, pull a model
        (<code>ollama pull {st.session_state.model}</code>), then chat. API:
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            full_response = "❌ Authentication failed. Check `OLLAMA_API_KEY` if your Ollama instance requires it."
            ...
            if "connection" in err or "refused" in err or "connect" in err:
                ...
                full_response = (
                    ...
                    f"Run `ollama pull {st.session_state.model}` first."
                )
            else:
